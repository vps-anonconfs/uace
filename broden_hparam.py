...
if __name__ == '__main__':
    """
    The purpose of this file is to evaluate the sensitivity of Y-CBM and O-CBM to regularization strength
    """
    ...
    for sub_dataset in ["pascal", "ade20k"]:
        ...
        for alpha in [1e-4, 1e-3, 1e-1, 1]:
            wts = cb_explainer.ycbm(fit_intercept=False, lasso_alpha=alpha)
            cbe[f'ycbm_alpha={alpha}'] = wts
            print(wts)
            
        for C in [1e-2, 0.1, 1, 10]:
            wts, extra_obj = cb_explainer.oikarinen_cbm(fit_intercept=False, return_concept_reprs=True, lasso_C=C)
            cbe[f'ocbm_C={C}'] = wts
            print(wts)
        
        model_dir = "lightning_logs"
        with open(f"{model_dir}/broden_hparam_{sub_dataset}.pkl", "wb") as f:
            pickle.dump(cbe, f)
